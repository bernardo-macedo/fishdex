<template>
  ...
  <div class="max-w-xl mx-auto px-4 py-8">
    <div class="flex items-center gap-3 mb-8">
      <router-link to="/" class="text-ocean-600 hover:text-ocean-800 transition-colors">
        ← Back
      </router-link>
      <h1 class="text-2xl font-bold text-slate-800">Log a catch</h1>
    </div>

    <form @submit.prevent="handleSubmit" class="space-y-5 bg-white rounded-2xl shadow-md p-6">
      ...
      <div>
        ...
        >
          ...
          <div v-else class="flex flex-col items-center justify-center h-full text-slate-400">
            ...
            <span class="text-sm mt-2">Tap to add photo</span>
          </div>
          <input
            ref="photoInput"
            type="file"
...
function onCatalogSelect() {
  ...
  if (fish) {
    ...
    // Pre-fill name only if still empty
    if (!form.value.name) {
      form.value.name = fish.name
    }
  } else {
    form.value.species = ''
